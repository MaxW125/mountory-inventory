...
const UNITS = ["kg", "pcs", "rolls", "sheets", "tubes", "liters", "meters"];
...
export default function AddMaterialModal({ onClose, onAdd, onCreate }) {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
          <Field label="Material Name *">
            <Input value={form.name} onChange={(e) => set("name", e.target.value)} className="h-9 text-sm" placeholder="e.g. PLA Filament – Black" />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Type">
              <Input value={form.type} onChange={(e) => set("type", e.target.value)} className="h-9 text-sm" placeholder="e.g. Filament" />
            </Field>
            <Field label="Color">
              <Input value={form.color} onChange={(e) => set("color", e.target.value)} className="h-9 text-sm" placeholder="e.g. Black" />
            </Field>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Brand">
              <Input value={form.brand} onChange={(e) => set("brand", e.target.value)} className="h-9 text-sm" placeholder="e.g. eSUN" />
            </Field>
          ...
          <div className="grid grid-cols-3 gap-3">
            <Field label="On Hand">
              <Input type="number" min="0" value={form.quantity_on_hand} onChange={(e) => set("quantity_on_hand", e.target.value)} className="h-9 text-sm" placeholder="0" />
            </Field>
            ...
            <Field label="Reorder Pt">
              <Input type="number" min="0" value={form.reorder_point} onChange={(e) => set("reorder_point", e.target.value)} className="h-9 text-sm" placeholder="0" />
            </Field>
          </div>
          <Field label="Unit">
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => { setAddingUnit(true); setManagingUnits(false); }}
              ...
              <button
                type="button"
                onClick={() => { setManagingUnits((v) => !v); setAddingUnit(false); }}
                className="h-9 w-9 flex items-center justify-center rounded-md border border-input bg-transparent text-muted-foreground hover:text-foreground hover:bg-muted transition-colors shrink-0"
                title="Manage units"
              >
            ...
            {addingUnit && (
              <div className="flex gap-2 mt-2">
                <Input
                  ...
                  onKeyDown={(e) => { if (e.key === "Enter") confirmNewUnit(); if (e.key === "Escape") setAddingUnit(false); }}
                  className="h-8 text-sm flex-1"
                  placeholder="New unit (e.g. boxes)"
                />
                <Button size="sm" onClick={confirmNewUnit} className="h-8 px-3">Add</Button>
                <Button size="sm" variant="outline" onClick={() => setAddingUnit(false)} className="h-8 px-3">Cancel</Button>
              </div>
            )}
            {managingUnits && (
              <div className="mt-2 rounded-md border border-border bg-muted/30 p-2 space-y-1">
                ...
                {units.map((u) => (
                  <div key={u} className="flex items-center justify-between rounded px-2 py-1 hover:bg-muted text-sm">
                    <span className="text-foreground">{u}</span>
                    <button onClick={() => removeUnit(u)} className="text-muted-foreground hover:text-destructive transition-colors ml-2">
                      <X className="h-3.5 w-3.5" />
                    </button>
                  </div>
                ))}
                <Button size="sm" variant="outline" className="w-full h-7 mt-1 text-xs" onClick={() => setManagingUnits(false)}>Done</Button>
              </div>
            )}
          </Field>
        </div>
